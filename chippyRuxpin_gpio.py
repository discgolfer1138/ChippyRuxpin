...
class GPIO:
    ...
    def setup(self,pin):
        ...
        cmd = "echo \"out\" > /sys/class/gpio/gpiochip" + str(pin) + "/direction" 
        ...
        if ( self.pins != None ):
            for pinObject in self.pins:
                if(pinObject[0]==pin and pinObject[1]!=val):
                    pinObject[1]=val
                    cmd = "echo " + str(val) + " > /sys/class/gpio/gpiochip" + str(pinObject[0]) + "/value"
                    subprocess.Popen(cmd,shell=True, stdout=subprocess.PIPE)
        
    def cleanup(self):
        for pinObject in self.pins:
            cmd = "echo 0 > /sys/class/gpio/gpiochip" + str(pinObject[0]) + "/value"
            subprocess.Popen(cmd,shell=True, stdout=subprocess.PIPE)
